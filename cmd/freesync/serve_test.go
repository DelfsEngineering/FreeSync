...
func TestServeHandler_Healthz(t *testing.T) {
	h := newServerHandler("cfg.json", "state.json", "", true, log.New(io.Discard, "", 0), func(context.Context, string, string, bool, *log.Logger) error {
		return nil
	})
...
func TestServeHandler_RunUnauthorized(t *testing.T) {
	h := newServerHandler("cfg.json", "state.json", "secret", true, log.New(io.Discard, "", 0), func(context.Context, string, string, bool, *log.Logger) error {
...
func TestServeHandler_RunApplyOverride(t *testing.T) {
	...
	h := newServerHandler("cfg.json", "state.json", "", true, log.New(io.Discard, "", 0), func(_ context.Context, _, _ string, apply bool, _ *log.Logger) error {
		calledApply = apply
		return nil
	})
	srv := httptest.NewServer(h)
...
func TestServeHandler_RunConflict(t *testing.T) {
	...
	h := newServerHandler("cfg.json", "state.json", "", true, log.New(io.Discard, "", 0), func(_ context.Context, _, _ string, _ bool, _ *log.Logger) error {
		started <- struct{}{}
		<-release
		return nil
	})
